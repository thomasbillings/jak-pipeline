...
- "add UAT before main" / "add Storybook previews per PR" on a project that already has `scrum-master-pipeline` installed.
...
- Plain `scrum-master-pipeline` install — that skill is the prerequisite, this is the layer above it.
...
2. **Plan 0 — Scrum Master pipeline scaffolding.** Absorbed from the formerly separate `scrum-master-pipeline` skill. Creates `plans/`, `agents/`, `agents/archive/`, `.claude/agents/`, `.claude/commands/`, `scripts/scrum-master/`. Installs the planner / plan-reviewer / dev-agent / scrum-master template files (never overwrites). Installs `tick.sh`, `dispatch.sh`, `lib.sh`, `check-plan.sh`. Appends the unified gitignore template (scrum-master + jak-pipeline entries). Plan-repo mode prompt — non-interactive via `JAK_PLAN_REPO=<owner>/<repo>` + optional `JAK_PROJECT_NAME=<name>`.
...
5. **Plan 3 — Jira integration.** Copies the transition helper, drift reconciliation pass, retry queue drain, and tick-extension into `<target>/scripts/jak-pipeline/jira/`; templates `<target>/.claude/jira/.env`; appends the `jak_pipeline_jira_tick_pass` source line to `scripts/scrum-master/tick.sh`.
...
| Plan       | Scope                                                                                                                         | Status                  |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| **Plan 0** | Scrum Master-pipeline scaffolding — planner / plan-reviewer / dev-agent / scrum-master + tick.sh / dispatch.sh / lib.sh / check-plan.sh + gitignore template. Absorbed from the formerly-separate `scrum-master-pipeline` skill. | **delivered** (PR-J) |
